...
const MetricBox: React.FC<MetricBoxProps> = ({
    ...
    return (
        <div className={`w-full ${additionalClass}`}>
            <Card className={`group h-full ${isHighlighted ? 'border-2 border-green-500 relative' : ''}`}>
                {isHighlighted && (
                    <div className="absolute -inset-0.5 bg-gradient-to-r bg-green-300  rounded-lg blur opacity-30 group-hover:opacity-100 transition duration-1000"></div>
                )}
                <CardContent className="p-2 sm:p-3 h-full w-full flex flex-col items-center justify-center relative space-y-1">
                    ...
                    />
                    <p className="font-semibold text-xs sm:text-sm text-center line-clamp-1">{label}</p>
                    {value && (
                        <p className={`text-sm sm:text-base md:text-lg font-bold text-center line-clamp-1 ${isHighlighted ? 'text-green-600' : ''}`}>
                            {value}
                        </p>
                    )}
                    {subValue && (
...
const ConnectionLine: React.FC<ConnectionLineProps> = ({ from, to, animated = false }) => {
    ...
    return (
        <line
            ...
            x2={to.x}
            y2={to.y}
            className={animated ? "stroke-green-500 stroke-1" : "stroke-gray-300 stroke-1"}
            strokeDasharray="4"
...
export function SolarDashboard() {
    const metrics = [
        ...
        },
        {
            ...
            value: "233.3 V",
            isHighlighted: false,
            hoverIconColor: "text-yellow-500",
        ...
        },
        {
            ...
            label: "Charged",
            isHighlighted: false,
            subValue: "charged", // Assuming it's charged for demonstration
                    ...
                    <div className="relative p-2 sm:p-4 md:p-8">
                        ...
                        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                            <div className="relative">
                                <div className="absolute -inset-1 bg-green-500 rounded-full animate-pulse opacity-75"></div>
                                <div className="w-12 sm:w-16 md:w-20 h-12 sm:h-16 md:h-20 rounded-full bg-green-500 flex items-center justify-center relative z-10">
